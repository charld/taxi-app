...
export async function GPS_permission() {
  try {
    const granted = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
      {
        title: "GPS Permission",
        message: "This App needs access to your GPS "
      }
    );
...
export default class Driver extends Component {
  ...
  findPassengers() {
    if (!this.state.lookingForPassengers) {
      ...
      this.socket = socketIO.connect("http://10.0.0.130:3000");
  ...
  acceptPassengerRequest() {
    //send friver location to passengerRequest
    ...
    if (Platform.OS === "ios") {
      ...
      Linking.openURL(
        `https://www.google.com/dir/?api=1&destination=${
